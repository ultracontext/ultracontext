...
- Ignores: `~/.ultracontext/ignores/.ultracontextignore` for global rules and `~/.ultracontext/ignores/<source>/.ultracontextignore` for source-specific rules. `uc init` seeds these files; OpenClaw starts with conversations and workspace memory notes by default. Edit then `uc sync reset`.
