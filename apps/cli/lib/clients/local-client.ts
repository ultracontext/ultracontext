// =============================================================================
// LocalContextClient — the pure, BROWSER-SAFE ContextClient over a StorageAdapter
// + @ultracontext/core. ZERO node: imports: it never opens the adapter, never
// touches the fs — it only calls the engine. The node factory (./local) and the
// browser factory (./local-browser) each open an adapter their own way, then
// hand it here. Every targeted verb takes an EXPLICIT context id — there is no
// default context. The single 'local' project is ensured by the factory.
// =============================================================================
...
export class LocalContextClient implements ContextClient {
    constructor(
