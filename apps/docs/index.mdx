---
title: 'Introduction'
description: 'Same context. Everywhere.'
---
...
**Same context. Everywhere. Open source.**

What Claude Code knows, Codex doesn't. What your teammate is shipping right now? Your agent has no idea.

UltraContext captures every agent's context in realtime and makes it available to all of them. Continue a session in a different agent, or just ask what's happening.
...
|                            |                                                                          |
| -------------------------- | ------------------------------------------------------------------------ |
| **`uc` CLI**               | Local-first context versioning from your terminal. Create, append, get, update, delete, list — backed by SQLite, no server required. [Learn more](/fundamentals/cli). |
| **MCP Server**             | Share context everywhere. Built into the API, or run standalone via stdio. [Learn more](/fundamentals/mcp-server). |
...
|                          |                                                                     |
| ------------------------ | ------------------------------------------------------------------- |
| **Simple API**           | Core methods: create, append, get, update, delete. That's it.       |
...
<CardGroup cols={2}>
    ...
    <Card title='Guides' icon='book-open-cover' href='/guides/store-retrieve-contexts'>
