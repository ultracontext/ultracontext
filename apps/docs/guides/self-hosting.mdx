...
| **Atomic destroy** | Yes (real transactions) | Best-effort (sequential REST) |
| **Open source** | Yes | Yes |
...
<Note>Context destroy and batch-delete run as sequential REST calls on this provider — a mid-operation failure may leave partially-deleted state. Use the `postgres` provider for atomic destroy.</Note>
