...
Filter contexts by metadata fields stored at creation time. All filters are optional and can be combined.
...
// by machine
const { data } = await uc.get({ host: 'Mac-mini.local' });

// by time range
const { data } = await uc.get({ after: '2026-03-01T00:00:00Z', before: '2026-03-08T00:00:00Z' });

// combine filters
const { data } = await uc.get({ source: 'codex', host: 'Mac-mini.local', limit: 5 });
```
...
# by machine
result = uc.get(host="Mac-mini.local")

# by time range
result = uc.get(after="2026-03-01T00:00:00Z", before="2026-03-08T00:00:00Z")

# combine filters
result = uc.get(source="codex", host="Mac-mini.local", limit=5)
```
</CodeGroup>

| Filter | Description |
|--------|-------------|
| `source` | Agent source (`claude`, `codex`, `openclaw`) |
| `user_id` | User identifier |
| `host` | Machine hostname |
| `project_path` | Project directory path |
| `session_id` | Session identifier |
| `after` | Only contexts created after this ISO 8601 timestamp |
| `before` | Only contexts created before this ISO 8601 timestamp |

<Info>Metadata filters use a GIN index on the `metadata` JSONB column for fast lookups.</Info>
