...
Forking is `uc context create --from <id>` — the same `create({ from })` shape, no separate
`fork` verb. It prints the new context id.

```bash bash
# full fork/clone
fork=$(uc context create --from ctx_abc123)

# at a past version / index / timestamp (each requires --from)
uc context create --from ctx_abc123 --version 1
uc context create --from ctx_abc123 --at 5
uc context create --from ctx_abc123 --before 2024-01-10T10:30:00Z

# tag the fork with --meta (repeatable, attaches CONTEXT metadata)
uc context create --from ctx_abc123 --version 1 --meta reason=rollback --meta ticket=JIRA-123
```
