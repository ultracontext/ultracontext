...
- `uc.delete(contextId, ids)` — **soft delete** of messages. Creates a new version; the deleted messages are preserved in prior versions and can be recovered via `uc.get(contextId, { version: N })`.
- `uc.destroy(contextId)` — **hard delete** of the entire context. All versions, messages, and history are permanently removed. Irreversible.
</Info>
...
<Warning>This is irreversible. The context, all its versions, and all messages are permanently deleted.</Warning>
