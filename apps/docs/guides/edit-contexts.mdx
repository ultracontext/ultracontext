...
<Info>
**Soft delete vs hard delete — two different operations.**
...
- **Hard delete** — `uc.destroy(contextId)` or the unified `uc.delete(contextId, { permanent: true })`. Wipes the entire context, all versions, all messages. Irreversible.

Both hard-delete forms are equivalent; `destroy` exists for clarity at call sites (easy to grep, hard to invoke by accident), the `permanent: true` param exists for developers who prefer a single `delete` method.
</Info>
...
// canonical — clear at call sites, easy to grep
await uc.destroy('ctx_abc123');

// or unified delete with permanent flag
await uc.delete('ctx_abc123', { permanent: true });

// with audit metadata (both forms accept it)
await uc.destroy('ctx_abc123', { metadata: { reason: 'GDPR request', author: 'alice' } });
...
# canonical
result = uc.destroy("ctx_abc123")

# or unified delete with permanent flag
result = uc.delete("ctx_abc123", permanent=True)

# with audit metadata
result = uc.destroy("ctx_abc123", metadata={"reason": "GDPR request", "author": "alice"})
# {"deleted": True, "id": "ctx_abc123", "metadata": {"reason": "GDPR request", "author": "alice"}}
```
</CodeGroup>
...
Delete multiple contexts in one call.
...
const { results } = await uc.batchDelete(['ctx_abc123', 'ctx_def456']);
...
result = uc.batch_delete(["ctx_abc123", "ctx_def456"])
...
<Tip>Batch delete returns results for each context individually, so partial failures don't block the entire operation.</Tip>
