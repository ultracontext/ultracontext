...
async function installSkill(extractDir) {
  ...
  if (!home) return;
  const defaults = `${home}/.claude/skills ${home}/.agents/skills ${home}/.openclaw/skills ${home}/.hermes/skills`;
...
async function main() {
  ...
  try {
    ...
    const home = process.env.HOME || process.env.USERPROFILE || "";
    const skillTargets = (process.env.ULTRACONTEXT_SKILL_TARGETS || `${home}/.claude/skills ${home}/.agents/skills ${home}/.openclaw/skills ${home}/.hermes/skills`)
      .split(/\s+/)
      .filter(Boolean);
