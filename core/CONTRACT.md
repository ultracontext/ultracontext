...
- **Mode rule**: `mode ?? (apiKey ? 'remote' : 'local')` — explicit mode wins. Remote is OUT of 2.0: in a real app (Next.js etc.) the user already HAS a server — the SDK runs there, native, and their frontend talks to their app like with any database. The rule + config shape stay reserved so remote (hosted) lands additively later. The browser NEVER embeds the engine (sandboxed, evictable, invisible storage betrays Ownership + Transparency); wasm stays demoted to optionality.
...
window is real) · remote/hosted mode itself (until then, browser/edge
consume via the USER's own server — the Next.js pattern) ·
compaction/splice (range-replace as one head) ·
