...
`core/` is the reusable Rust implementation boundary. If behavior must be
identical across JS, Python, CLI, local, remote, and mounts, it belongs here.
...
The top-level `cli/` crate consumes this core and owns terminal UX, project
configuration, install flow, and the local NFS mount surface.
